...
async function toFormFilePart(file: UploadFileInput): Promise<Blob> {
  if (Platform.OS === 'web') {
    ...
    return new File([blob], file.name || 'photo.jpg', { type: file.type || blob.type || 'image/jpeg' });
  }
...
function mapProduct(dto: BackendProductDto): Product {
  const imageUrl = resolveMediaUrl(dto.photoPath);
  if (__DEV__) {
    console.log('[MAP] mapProduct photoPath ->', dto.photoPath, '| imageUrl ->', imageUrl);
  }
  return {
    id: String(dto.id),
    name: dto.name,
...
export const realApi: Api = {
  ...
  products: {
    ...
    uploadImage: async (file) => {
      ...
      const fileName = file.name || file.uri.split('/').pop() || `product_${Date.now()}.jpg`;
      const match = /\.(\w+)$/.exec(fileName);
      const fileType = file.type || (match ? `image/${match[1]}` : 'image/jpeg');

      if (Platform.OS === 'web') {
        const blob = await toFormFilePart({ uri: file.uri, name: fileName, type: fileType });
      ...
      const res = await httpPost<{ url: string }>('/products/images', form);
      if (__DEV__) console.log('[UPLOAD] /products/images response ->', res);
